...
//        //create the url with URL
//        let url = URL(string: "https://guitar-mult-backend.herokuapp.com/"+url)!
//
//        //create the session object
//        let session = URLSession.shared
//
...
//    }
    ...
    static func getAPIRequest(_ url : String, _ userID : String) {
        //create the url with URL
        let url = URL(string: "https://guitar-mult-backend.herokuapp.com/"+url+"/"+userID as String)! //change the url
          ...
          guard let httpResponse = response as? HTTPURLResponse,
            ...
            if let data = data {
                do {
                    if let json = try JSONSerialization.jsonObject(with: data, options: .mutableContainers) as? [String: Any] {
                        print(json)
                        if json["dataUpdate"] != nil {
                            if json["dataUpdate"] as! Bool == true {
                                for (key,val) in json {
                                    if key != "dataUpdate" && key != "__v" {
                                        print(key)
                                        userLevelData.updateValueOnAPIRequest(key, val as! String)
                                    }
                                }
